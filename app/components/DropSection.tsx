...
function DropSection({ title, display_title, children, style, shortcutSwitches, active: default_active = false }: { title: string, display_title?: string, children: React.ReactNode, style?:string, shortcutSwitches?: string[], active ?: boolean}) {
    ...
    <section className={`transition-all accordion-item border bg-white border-gray-200 shrink-0`} data-accordion-default={default_active}>
            {/* <div className="border-0"> */}
                ...
                focus:outline-none
                  min-h-[10vh]`}>
                    <span className=''>
                        <h2 className='inline'>{title + (display_title ? ` : ${display_title}` : '')}</h2>
